...
export default function App() {
  ...
  // --- NEW: trigger load test ---
  async function runLoadTest(duration, clients) {
    try {
      const res = await fetch(`${API_URL.replace('/api','')}/api/load-test`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ duration, clients })
      })
      const data = await res.json()
      alert(`Load test started: ${clients} clients for ${duration}s`)
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial' }}>
      ...
      {/* --- NEW SECTION: Load Testing Buttons --- */}
      <h2>Load Test</h2>
      <button onClick={() => runLoadTest(30, 10)}>🚦 Low Traffic (10 clients / 30s)</button>
      <button onClick={() => runLoadTest(60, 50)}>🚀 High Traffic (50 clients / 60s)</button>
    </div>
  )
}
